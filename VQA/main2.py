...
def question():
    ...
    if request.method == "POST":
        ...
        if type==0:
            model_QA=keras.models.load_model('model/model_new/Object_18000/18000_object_VGG_32.h5')
            maxlen=34
            all_answer=pd.read_csv('model/model_new/Object_18000/Object_18000_answer.csv')
            all_answer=all_answer['0'].values
            tokenizer=pickle.load(open("model/model_new/Object_18000/18000_object_VGG.pkl_32", 'rb'))
        elif type==1:
            #model_QA=keras.models.load_model('model/number_VGG_56.h5')
            model_QA=keras.models.load_model('model/model_new/Number_5000/Number_5000_MobileV2Net_66.h5')
            maxlen=38
            all_answer=pd.read_csv('model/number.csv')
            all_answer=all_answer['0'].values
            #tokenizer=pickle.load(open("model/number_VGG_56.pkl", 'rb'))
            tokenizer=pickle.load(open("model/model_new/Number_5000/Number_5000_MobileV2Net_66.pkl", 'rb'))
        elif type==2:
